...
# This script reformat time series data by HDI and Age.group
...
col1 <- read.csv("PAHO_QuarterTS_HDIcat_15Dec2018_SubChapters_Age.group.csv", as.is = T)
...
# Check
unique(country_code) # Should be "ar" "br" "co" "dr" "ec" "gy" "hr" "mx" "nc" "pr"
unique(age_cat) # Should be  "0" "1" "2" "3"
...
age_cat_char[age_cat=='0']<-'<2m'
age_cat_char[age_cat=='1']<-'2-11m'
...
#------------------------------------------------------------------------------#
# Reformat time series data by HDI and Age.group
#------------------------------------------------------------------------------#
...
select_agegrp <- c('2-11m','12-23m','24-59m')
name_agegrp <- "2-59m"
# Country-level
ntnl <- agg_ntnl(col1, select_agegrp, name_agegrp)
# HDI-level
...
setwd("~/Desktop/PAHO_Mortality_New/Data")
write.csv(all.agg, 'Quarterly_ByAgeGroup1_ByHDI_15Dec2018.csv', row.names=F)
