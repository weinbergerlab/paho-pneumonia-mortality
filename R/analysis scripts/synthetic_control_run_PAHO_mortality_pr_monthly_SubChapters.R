...
country       <- 'pr_SubChpt_BestModel_woPandemic' #Country or region name.
n_seasons     <- 12       #Number of months (seasons) per year. 12 for monthly, 4 for quarterly, 3 for trimester data.
exclude_covar <- c('J09_J18_prim','J00_J99_prim' ,
                   'J12_J18_any', 
                    "pandemic",
                   'J13_any',
                   'possible_pneumo_code',"population" ,"acm_noj_nodiarr_prim",
                   'A00_A09_prim', 'A30_49_prim', 'B95_B98_prim', 'G00_G09_prim', 'H65_H75_prim',
                   'F00_F99_prim', "O00_O99_prim", 
                   "I00_I99_prim", "I00_I02_prim", "I05_I09_prim", "I10_I15_prim",
                   "I20_I25_prim", "I26_I28_prim", "I30_I52_prim", "I60_I69_prim",
                   "I70_I79_prim", "I80_I89_prim", "I95_I99_prim", 
                   "R00_R99_prim", "R00_R09_prim", "R10_R19_prim", "R20_R23_prim",
                   "R25_R29_prim", "R30_R39_prim", "R40_R46_prim", "R47_R49_prim",
                   "R50_R69_prim", "R70_R79_prim", "R80_R82_prim", "R83_R89_prim",
                   "R90_R94_prim", "R95_R99_prim")      #User-defined list of covariate columns to exclude from all analyses.
...
file_name="PAHO all age cuts_IncludingJ12_18_any.csv"
...
prelog_data<-prelog_data[substr(prelog_data$age_group,1,2)=='pr',] 
...
# NOTE: this part should be corrected when we use population in the model. (Population is also zero for missing months, which is not right.)
...
# NOTE: missing observations do not exist any more, as it was corrected above.
nrow(prelog_data)
...
intervention_date <- as.Date('2009-07-31') #Indicates the date of intervention in the data.
end_date          <- as.Date('2014-12-01') #Indicates the date of the last data point.
pre_period        <- as.Date(c('2005-01-01', '2009-07-31')) #Range over which the data is trained for the CausalImpact model.
post_period       <- as.Date(c('2009-08-01', '2014-12-01')) #Range from the intervention date to the end date.
eval_period       <- as.Date(c('2010-08-01', '2014-12-01')) #Range over which rate ratio calculation will be performed.
