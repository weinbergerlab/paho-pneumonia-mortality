...
country       <- 'hr_SubChpt_BestModel_woPandemic' #Country or region name.
...
file_name="Quarterly_ByAgeGroup1_ByHDI_15Dec2018.csv"
...
prelog_data<-prelog_data[substr(prelog_data$age_group,1,2)=='hr',]
...
nrow(prelog_data)
nrow(prelog_data[n.obs2==max(n.obs2) & n.deaths2>400,])
prelog_data<-prelog_data[n.obs2==max(n.obs2) & n.deaths2>400,]
...
# prelog_data <- prelog_data[which(prelog_data$age_group=="hr 2-11m A" | 
#                                    prelog_data$age_group=="hr 12-23m A"),]
...
pre_period        <- as.Date(c('2007-01-01', '2010-12-31')) #Range over which the data is trained for the CausalImpact model.
...
# plot(J12_J18_prim ~ as.Date(monthdate),
#      data=prelog_data[which(prelog_data$age_group==unique(prelog_data$age_grou)[9]),],
#      type="l",bty="l",col="blue")

# J12_J18_prim > 1 for all age groups after filtering
