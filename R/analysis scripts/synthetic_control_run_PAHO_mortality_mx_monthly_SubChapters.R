...
country       <- 'mx_SubChpt_BestModel_woPandemic' #Country or region name.
...
prelog_data<-prelog_data[substr(prelog_data$age_group,1,2)=='mx',]


####### For sparse data with missing months #######

# Some months in some age groups miss rows
length(prelog_data$monthdate[which(prelog_data$age_group==unique(prelog_data$age_group)[1])]) / 12 # Shoud be 9 years but not
length(prelog_data$monthdate[which(prelog_data$age_group==unique(prelog_data$age_group)[2])]) / 12
length(prelog_data$monthdate[which(prelog_data$age_group==unique(prelog_data$age_group)[3])]) / 12
length(prelog_data$monthdate[which(prelog_data$age_group==unique(prelog_data$age_group)[4])]) / 12

# Add rows with zeros for missing months
...
for (i in 1:length(AgeGrp)) {
  ...
  if (length(dt$monthdate) != CorrectNumMonths) {
    ...
    prelog_data <- prelog_data[-c(which(prelog_data$age_group==AgeGrp[i])),]
    prelog_data$monthdate <- as.Date(prelog_data$monthdate)
    prelog_data <- rbind(prelog_data, FixMonths)
  } 
}

########################################

#how many timepoints per group?
...
prelog_data <- prelog_data[substr(prelog_data$age_group,
                                  nchar(as.character(prelog_data$age_group)), 
                                  nchar(as.character(prelog_data$age_group)))=="A",]
...
post_period       <- as.Date(c('2008-01-01', '2015-12-01')) #Range from the intervention date to the end date.
eval_period       <- as.Date(c('2009-01-01', '2015-12-01')) #Range over which rate ratio calculation will be performed.
