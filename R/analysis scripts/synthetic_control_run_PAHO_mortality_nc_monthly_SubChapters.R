...
country       <- 'nc_SubChpt_BestModel_woPandemic' #Country or region name.
...
prelog_data<-prelog_data[substr(prelog_data$age_group,1,2)=='nc',]
...
intervention_date <- as.Date('2011-12-31') #Indicates the date of intervention in the data.
end_date          <- as.Date('2015-12-01') #Indicates the date of the last data point.
pre_period        <- as.Date(c('2005-01-01', '2011-12-31')) #Range over which the data is trained for the CausalImpact model.
post_period       <- as.Date(c('2012-01-01', '2015-12-01')) #Range from the intervention date to the end date.
